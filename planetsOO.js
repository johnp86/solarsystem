$(document).ready(function() {
  ...
  var sun = s.circle(600, 360, 40).attr({
    fill: "#FDB813"
  });

  function Circle(x, y, radius, fill) {
    this._circle = s.circle(x, y, radius).attr({
      fill: fill
    });
  ...
  Planet.prototype.createMoons = function() {
    ...
    var moonX, moonY, moon;
    for (i = 0; i < moonNum; i++) {
      moonX = planetCenter.x + planetRadius + 4;
      moonY = planetCenter.y + planetRadius + 4;

      moon = new Circle(moonX, moonY, 1, 'grey');

      // Moon obit
      function moonOrbit() {
        moon.transform("r" + (moonTransform * i) + ", " + planetCenter.x + ", " + planetCenter.y);
        moon._circle.animate({
          transform: "r" + (moonTransform * i + 360) + ", " + planetCenter.x + ", " + planetCenter.y
        }, 20000, mina.linear, moonOrbit.bind(moon));
      };
      moonOrbit();
  ...
  saturn.ring1 = s.ellipse(360, 360, 16, 2).attr({
    stroke: "#75673d",
    "stroke-width": "2px",
    fill: 'none'
  });
  saturn.ring2 = s.ellipse(360, 360, 20, 4).attr({
    stroke: "#75673d",
    "stroke-width": "2px",
    fill: 'none'
  });
  saturn.core = s.circle(360, 360, 9).attr({
    fill: "#75673d"
  });
